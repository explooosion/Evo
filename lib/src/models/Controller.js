...
class Controller {
    ...
    renameToTemp() {
        fs.readdirSync(this.gamePath)
            .filter(value => { return path.extname(value) === this.extnameKom })
            .map(value => { return path.resolve(this.gamePath, value) })
            .forEach(value => {
    ...
    renameToKom() {
        fs.readdirSync(this.gamePath)
            .filter(value => { return path.extname(value) === this.extnameEvo })
            .map(value => { return path.resolve(this.gamePath, value) })
            .forEach(value => {
